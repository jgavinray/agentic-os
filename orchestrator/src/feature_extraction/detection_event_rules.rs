...
use crate::feature_detection_event_type_rules::{
    append_assistant_message_tags, append_context_pack_tags, append_trajectory_result_tags,
};
...
use crate::feature_extraction_metadata::{
    bool_path, string_path, summarizer_shares_litellm_upstream,
};
...
pub(crate) fn deterministic_tags_for_event(
    ...
    if event_type == "context_pack" {
        append_context_pack_tags(&mut tags, metadata, source_override);
    }

    if event_type == "assistant_message" {
        append_assistant_message_tags(&mut tags, summary, metadata, source_override);
    }

    if event_type == "trajectory_result" {
        append_trajectory_result_tags(&mut tags, metadata, source_override);
    }
