...
pub async fn messages(
    ...
    if let Some(envelope) = envelope_guidance.as_deref() {
        crate::system_context::inject_system_context_anthropic(&mut req, envelope);
    }
    ...
    if let Some(nudge) = validation_gate.nudge.as_deref() {
        crate::system_context::inject_system_context_anthropic(&mut req, nudge);
    }
    ...
    if let Some(plan) = execution_plan {
        crate::system_context::inject_system_context_anthropic(&mut req, &plan.guidance);
    }
    ...
    if let Some(guidance) = runtime_prompt_intervention.guidance.as_deref() {
        crate::system_context::inject_system_context_anthropic(&mut req, guidance);
    }
